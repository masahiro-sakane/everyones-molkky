...
describe.skipIf(!hasDb)('matchService 統合テスト', () => {
  ...
  it('試合を作成できる', async () => {
    ...
    const match = await createMatch({ matchType: 'TEAM', teamIds: [teamAId, teamBId], totalSets: 1 })
  ...
  it('2チーム未満で試合作成はエラーになる', async () => {
    ...
    await expect(createMatch({ matchType: 'TEAM', teamIds: [teamA.id], totalSets: 1 })).rejects.toThrow()
  })
