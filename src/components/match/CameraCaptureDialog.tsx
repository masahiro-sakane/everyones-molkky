...
import type { AdviceContext, ThrowerProfile } from '@/types/vision'
import { DEFAULT_THROWER_PROFILE } from '@/types/vision'
...
export function CameraCaptureDialog({ ctx, onClose }: CameraCaptureDialogProps) {
  ...
  const [thrower, setThrower] = useState<ThrowerProfile>(DEFAULT_THROWER_PROFILE)
  const dialogRef = useRef<HTMLDivElement>(null)
  const fileInputRef = useRef<HTMLInputElement>(null)

  const effectiveCtx: AdviceContext = { ...ctx, thrower }
    ...
    >
      ...
      >
        ...
        <div className="px-4 pb-4 flex flex-col gap-3">
          {/* 投擲者プロフィール入力 */}
          {!result && (
            <div className="flex flex-col gap-2 p-3 bg-neutral-50 border border-neutral-200 rounded-lg">
              <p className="text-[11px] font-semibold text-neutral-500 uppercase tracking-wide">投擲者プロフィール</p>
              <div className="flex items-center gap-2">
                <span className="text-xs text-neutral-600 shrink-0 w-16">スキル</span>
                <div className="flex gap-1">
                  {(['beginner', 'intermediate', 'advanced'] as const).map((lvl) => {
                    const label = lvl === 'beginner' ? '初心者' : lvl === 'intermediate' ? '中級者' : '上級者'
                    return (
                      <button
                        key={lvl}
                        type="button"
                        onClick={() => setThrower((t) => ({ ...t, level: lvl }))}
                        className={[
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  ...
                  checked={thrower.canThrowVertical}
                  onChange={(e) => setThrower((t) => ({ ...t, canThrowVertical: e.target.checked }))}
                />
                <span className="text-xs text-neutral-600">縦投げができる</span>
              </label>
            </div>
          )}
          ...
          {/* 解析結果 */}
          {result && (
            <AdviceResultPanel analysisResult={result} ctx={effectiveCtx} />
          )}
