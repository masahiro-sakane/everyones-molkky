...
| skittlesKnocked | number[] | ✓ | 倒したスキットルの番号配列（後述の規約参照） |
...
|------|------|------|
| `[]` | ミス（0点） | 0 |
| `[7]` | スキットル7番のみを1本倒した | 7（番号がそのまま得点） |
| `[3, 10, 4]` | 3,10,4番のスキットルを倒した（番号特定あり） | 3（本数） |
| `[0, 0, 0]` | 3本倒した（番号特定なし、UI からの複数本モード入力） | 3（本数） |

センチネル値 `0` は「番号特定なしの複数本入力」を表す。`0` を含む配列では番号重複チェックは省略される。

**得点計算ルール**
- 0本（空配列）: 0点
- 0 を含む配列: 配列長 = 倒した本数 = 得点
- 1本（長さ1で 0 を含まない）: 要素の数値 = 得点
- 複数本（長さ2以上で 0 を含まない）: 配列長 = 倒した本数 = 得点
- フォルトあり: skittlesKnocked は無視されて 0 点
